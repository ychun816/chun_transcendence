 ...
 */
export default async function chatWebSocketRoutes(fastify, prisma) {
    ...
    fastify.get("/ws/chat", { websocket: true }, async (connection, req) => {
        ...
        connection.on("message", async (message) => {
            try {
                ...
                switch (data.type) {
                    case "direct_message":
                        await handleDirectMessage(data, username, user.id, prisma);
                        break;
                    case "block_user":
                    ...
                    case "get_conversations":
                        await handleGetConversations(user.id, connection, prisma);
                        break;
                    case "get_messages":
                        console.log("🔧 Processing get_messages with data:", data);
                        await handleGetMessages(data, user.id, connection, prisma);
                        break;
                    case "get_online_users":
                        await handleGetOnlineUsers(connection, prisma, username);
                        break;
                    default:
 ...
 */
async function handleDirectMessage(data, senderUsername, senderId, prisma) {
    ...
    try {
        ...
        if (isBlocked) {
            console.log("🔧 Sender is blocked by receiver ", senderUsername, "by", receiverUsername);
            sendToUser(senderUsername, {
                type: "error",
                message: "You cannot send messages to the user " +
                    receiverUsername +
                    " because you are " +
                    senderUsername,
            });
 ...
 * @param prisma - Prisma client
 */
async function handleBlockUser(data, blockerId, prisma) {
    ...
    try {
        ...
        if (activeConnections.has(usernameToBlock)) {
            sendToUser(usernameToBlock, {
                ...
                username: activeConnections.get(usernameToBlock)?.username || "",
            });
        }
    }
    catch (error) {
 ...
 * @param prisma - Prisma client
 */
async function handleUnblockUser(data, unblockerId, prisma) {
    ...
    try {
        ...
        if (activeConnections.has(usernameToUnblock)) {
            sendToUser(usernameToUnblock, {
                ...
                username: activeConnections.get(usernameToUnblock)?.username || "",
            });
        }
    }
    catch (error) {
 ...
 * @param prisma - Prisma client
 */
async function handleGetUserProfile(data, connection, prisma) {
    const { username } = data;
    try {
        const user = await prisma.user.findUnique({
            where: { username },
 ...
 * @param prisma - Prisma client
 */
async function handleGetConversations(userId, connection, prisma) {
    try {
 ...
 * @param prisma - Prisma client
 */
async function handleGetMessages(data, userId, connection, prisma) {
 ...
 * @param data - Data to send
 */
function sendToUserById(userId, data) {
 ...
 */
function broadcastToAll(data, excludeUsername) {
    for (const [username, conn] of activeConnections) {
 ...
 */
export function sendTournamentNotification(username, message) {
    sendToUser(username, {
 ...
 */
async function getOnlineUsersData(prisma, excludeUsername) {
    try {
        const onlineUsernames = Array.from(activeConnections.keys()).filter((u) => u !== excludeUsername);
 ...
 */
async function handleGetOnlineUsers(connection, prisma, currentUsername) {
    try {
        const onlineUsersData = await getOnlineUsersData(prisma, currentUsername);
        connection.send(JSON.stringify({
            type: "online_users",
            users: onlineUsersData,
        }));
